...
**Current version:** `1.34.0` (see `VERSION`)
...
2. **Data check + rev3 refresh** — ✅ done. The "bulletproof" rev3 dataset landed
   ...
   site dataset is emitted from it — **rev3 controls 100%**. See **DATA notes**.
3. **Display all the data** — ✅ done (PR #3). Full 15-yr history, expanded
   profile/compare, demographics-evolution chart, provenance markers.
4. **UX feedback rounds** — ✅ done: removed "is it worth it?" framing, lowercase α,
   visible left scrollbar, tooltip→body-level/viewport-clamped, self-serve Compare
   picker, map cost/admissions layers, intermediate responsive tier, **full school
   page compressed to a tight 3-across**.
5. **SEO / AI-crawl audit (P0+P1)** — ✅ done (see "Audit build" below).
6. **Blog** — ✅ done: static, data-driven `/blog/` + RSS, regenerated each build.
7. **Mobile.** Full mobile pass — layout, touch targets, tab bar, bottom-sheet
   panel, charts, phone perf. **Not started — the NEXT milestone. Needs a device.**

> ⚠ Everything since the data refresh lives on **PR #3**, not `main`. **No browser
> in the build env** — all validation is structural (JS parses, JSON-LD/RSS/sitemap
> well-formed, 272 sitemap URLs resolve, link audit, SVG guard). **Visual QA + a
> hard-reload on live (new service worker) are still wanted before/after merge.**

---

## Audit build (SEO / AI-crawl — generated by `scripts/build_school_pages.py`)

One generator now emits the whole crawlable surface from `data/exhibit-data.js` +
`data/exhibit_data.json.gz`. Run `python3 scripts/build_school_pages.py` after any
data change, then `stamp_version.py` + bump `VERSION`.

| Output | What |
|---|---|
| `school/<slug>.html` ×208 | 15-yr trajectory `<table>` (from gz) + `@graph` JSON-LD (CollegeOrUniversity + Dataset + FAQPage + BreadcrumbList) + visible FAQ + cohort caveat + cite/report + standout-stat `<title>` |
| `school/<slug>.md` ×208 | Markdown twin (cleanest LLM input); served `text/markdown` via `_headers` |
| `rankings/*.html` ×4 | highest-bar / best-value / cheapest / most-biglaw — prose lede + ranked table + `ItemList` |
| `state/<state>.html` ×51 | per-state hub — every school ranked + prose lede + `ItemList` |
| `blog/` + 3 posts + `feed.xml` | data-driven changelog + analyses; `BlogPosting`/`Blog` JSON-LD + RSS |
| `exhibit-dataset.json` | bulk open dataset (208 schools) |
| `sitemap.xml` | 272 URLs; `index.html` no-JS block gets national aggregates + ranked/state/blog links |

`robots.txt`/`llms.txt`/`_headers` already reference the dataset, feed, and `.md`
twins. Sitewide `WebSite`/`Dataset` JSON-LD + a homepage `FAQPage` ship in `index.html`.

**Deferred audit items:** SPA Match-Me a11y (labels/aria-live), competitor-comparison
pages, median-alongside-mean, PR RPP "not published" note, CI Lighthouse gate,
Cloudflare-native form. None blocking.

---

## rev3 emit pipeline

The data layer is regenerated from `exhibit.db` (rev3, 2011–2025, 203,811 facts):

| Script | Produces |
|---|---|
| `scripts/emit_from_rev3.py` | `data/exhibit-data.js` (`S`): current-cycle scalars + derived %s + 15-yr `*_trend` overlay + `_adj` provenance. `FORKS` dict toggles sector basis / closed-school population / trend rebuild (all on). Also sanitizes implausible `enr_trend` points → dashed gaps. |
| `scripts/emit_gz_from_rev3.py` | `data/exhibit_data.json.gz`: 2011–2026 history (rev3 core overlaid, v2 demographics/curriculum/raw preserved); adds the 4 inline-only closed schools. |

Both need `--db /path/to/exhibit.db` (DB is **not** in the repo; ships in
`exhibit.7z`). Full data→site rebuild order: `emit_from_rev3.py` →
`emit_gz_from_rev3.py` → `build_school_pages.py` → `stamp_version.py` → bump `VERSION`.

**Known data caveats:** Ohio Northern `schol_none_pct` 100.0 (bug in rev3 source);
`enr_trend` (total-JD) and `cond_*_trend` carried over (rev3 lacks them); `fac_trend`
rebuilt as FT+PT total.
...
ABA-accredited U.S. law school. Single-page app (vanilla JS, Leaflet map) plus a
generated crawlable layer (208 static school pages + .md twins, 51 state hubs, 4
ranked lists, a blog). Positioned around outcomes, true cost, and 15-year trajectory.
